function [pulses,threshpts,fig_handles]=CLmznps_pickpulse_semiauto_nodownsamp(ydetrend,yasls,yas2det,dmin,stderror,yas3det,plot_thresh,skipto,use_old_fig,fig_handles)
L=12650;%overall length of device 
szlength=800; %sizing channel length 
sqlength=2000; %squeeze channel length 
...
eventlength=11800; %(USED IN aretheytogether) approximate length (in data points) of complete cell event (all 3 zones), if this is too big you will miss events, if too small you'll catch same event more than once 
...
while i<=me 
        ...
        title(axb1,num2str(p))
         ...
         elseif isempty(choice) %choice ~=1 && choice ~=0
            ...
            threshgood=0;
            while threshgood==0
                redo=input('Do we need to redo threshpts?, type 1=yes, type 2=no');
                if isempty(redo)==1
                    threshgood=1;
                elseif redo==1
